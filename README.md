# 🚀 OpenClaw 完全指南

本地 AI 助手 | 你的智能工作伙伴

[![Star](https://img.shields.io/github/stars/tangyuan-dev/openclaw-tutorials?style=social)](https://github.com/tangyuan-dev/openclaw-tutorials/stargazers)
[![License](https://img.shields.io/github/license/tangyuan-dev/openclaw-tutorials)](LICENSE)
[![Last Commit](https://img.shields.io/github/last-commit/tangyuan-dev/openclaw-tutorials)](https://github.com/tangyuan-dev/openclaw-tutorials/commits)

> OpenClaw — 让 AI 成为你的同事，而非工具

## 📖 什么是 OpenClaw？

OpenClaw 是一个本地运行的 AI 助手框架，可以帮你完成：

- 🖥️ **自动化工作流** — 浏览器操作、文件管理、代码执行
- 💬 **多渠道接入** — Telegram、Discord、飞书、微信
- 🧠 **记忆系统** — 长期记忆 + 每日笔记，跨会话保持上下文
- 🔌 **技能生态** — 可扩展的技能市场，随装随用

## ✨ 特性

| 特性 | 说明 |
|------|------|
| 🏠 **本地优先** | 数据留在本地，保护隐私 |
| 🎯 **精准控制** | 细粒度权限，安全可控 |
| 🔄 **持续学习** | 记忆系统让 AI 越用越懂你 |
| 🛠️ **可扩展** | 技能市场，插件式架构 |

## 📚 教程目录

### 基础入门
- [01. 安装与配置](./docs/01-setup.md)
- [02. 第一个任务](./docs/02-first-task.md)
- [03. 渠道配置](./docs/03-channels.md)

### 进阶实战
- [04. 自定义技能](./docs/04-custom-skill.md)
- [05. 记忆系统](./docs/05-memory.md)
- [06. 工作流自动化](./docs/06-workflow.md)

### 最佳实践
- [07. 安全配置](./docs/07-security.md)
- [08. 性能优化](./docs/08-performance.md)
- [09. 案例集](./docs/09-cases.md)

## 🚀 快速开始

```bash
# 安装
npm install -g openclaw

# 初始化
openclaw init

# 启动
openclaw start
```

详细步骤见 [安装指南](./docs/01-setup.md)

## 🛠️ 技术栈

- **运行时**: Node.js
- **AI 接入**: OpenAI / Anthropic / Ollama / MiniMax
- **渠道**: Telegram, Discord, 飞书, WhatsApp, Signal
- **存储**: 本地文件系统 + SQLite

## 📦 相关项目

| 项目 | 描述 | Stars |
|------|------|-------|
| [openclaw](https://github.com/openclaw/openclaw) | 核心框架 | ⭐ |
| [openclaw-docs](https://github.com/openclaw/docs) | 官方文档 | ⭐ |

## 🤝 贡献

欢迎提交 Issue 和 PR！请先阅读 [贡献指南](./CONTRIBUTING.md)。

## 📄 License

MIT License — 详见 [LICENSE](./LICENSE)

---

<div align="center">

**⭐ Star 这个仓库，一起探索 AI 助手的无限可能！**

</div>
